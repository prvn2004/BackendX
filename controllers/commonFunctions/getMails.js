...
const {classifyFollowups, passThroughClassifyMails, shouldFollowuped} = require('../../controllers/gemini/classifyFollowup');

const getMessagesByDate = async (_id, useruid, query) => {
  try {
    ...
    // Sort emails by internalDate in ascending order
    emails.sort((a, b) => a.internalDate - b.internalDate);

    // Save emails to database
    for (const email of emails) {
      await saveMailToDatabase(email, useruid);
    }
  } catch (error) {
